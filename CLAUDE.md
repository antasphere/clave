...
- **Usage**: `store/usage-store.ts` shares one cache per provider (and Pi range) between the sidebar's foot and Usage settings, and for Claude one cache PER ACCOUNT (`claudeUsageStore(accountId)`). The foot follows the focused local session: the Claude account it runs on, Codex quota, or Pi's local tokens and recorded cost today. Panel refresh updates that same cache. The main process owns the accounts (`src/main/claude-accounts.ts`: a label plus a pasted `claude setup-token` token held encrypted by `safeStorage`, a `CLAUDE_CONFIG_DIR`, or the machine login), reads every account every five minutes (`usage-manager.ts`, `startPolling`, off under `--test-no-activate`) and pushes each read to every window. A token is inference-only and the OAuth usage endpoint refuses it, so a token account reads its windows off the `anthropic-ratelimit-unified-*` headers of a one-token Haiku message (about twenty tokens per read); folder and keychain accounts keep the endpoint. The token never leaves main: the spawn reads it by `claudeProfileId` (`accountTokenForSpawn`, Claude sessions only) into the process environment, and tmux carries it through `update-environment` (`TMUX_SESSION_ENV_VARS`, reconciled on a live server, since a shared server hands new sessions its own environment). `tests/e2e/claude-accounts.spec.mjs` proves the account by the token the session's own shell prints, never by a badge. `tightestWindow()` picks the closest reported cap, severity first, without hardcoding model names. Codex reads `account/rateLimits/read` through a short-lived `codex app-server` connection; normalize every returned bucket and derive durations from `windowDurationMins`, never from `primary`/`secondary` position. The foot shows percent **LEFT**, the pane percent used. `tests/e2e/provider-usage.spec.mjs` checks provider switching, shared refresh, delayed/error responses, and Pi's distinction from quota; `sidebar-foot.spec.mjs` covers the foot's geometry and brand.
